...
if (os.system("db2 connect to " + database +" > /dev/null") == 0):
    logs=str.split(os.popen("db2 connect to " + database + "; db2 'select int((float(total_log_used)/float(total_log_used+total_log_available))*100) from sysibmadm.snapdb'").read())
